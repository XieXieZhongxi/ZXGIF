...
+(void)createGIFformVideoURL:(NSURL *)videoURL completion:(void(^)(NSString * gifURL))completionBlock
{
    ...
    NSDictionary * frameProperties = [self framePropertiesWithDelayTime:0.1];
    ...
    dispatch_group_notify(gifQueue, dispatch_get_main_queue(), ^{
        // Return GIF URL
        completionBlock([[[NSString stringWithFormat:@"%@",gifURL] componentsSeparatedByString:@"file://"] lastObject]);
    });
}


+ (void)createGIFfromURL:(NSURL*)videoURL withFrameCount:(int)frameCount delayTime:(int)delayTime loopCount:(int)loopCount completion:(void(^)(NSString *GifURL))completionBlock {
...
+ (NSURL *)createGIFforTimePoints:(NSArray *)timePoints fromURL:(NSURL *)url fileProperties:(NSDictionary *)fileProperties frameProperties:(NSDictionary *)frameProperties frameCount:(int)frameCount gifSize:(GIFSize)gifSize{
    NSString * videoPath = [NSString stringWithFormat:@"%@",url];
    NSString * temporaryFile = [[[[videoPath componentsSeparatedByString:@"file://"] lastObject] componentsSeparatedByString:[videoPath pathExtension]][0] stringByAppendingString:@"gif"];
